...
def plot_performance_vs_opponent_elo(df):
    ...
    fig = px.box(df, x='PlayerResultString', y='EloDiff', title='Elo Advantage vs. Result',
                 labels={'PlayerResultString': 'Result', 'EloDiff': 'Player Elo - Opponent Elo'},
...
with col1: lichess_username = st.text_input("Lichess Username:", key="username_input", placeholder="e.g., DrNykterstein")
with col2: time_period = st.selectbox("Time Period:", options=list(TIME_PERIOD_OPTIONS.keys()), key="time_period_select")
with col3: selected_perf_type = st.selectbox("Game Type:", options=PERF_TYPE_OPTIONS_SINGLE, index=PERF_TYPE_OPTIONS_SINGLE.index(DEFAULT_PERF_TYPE), key="perf_type_select")
...
if isinstance(st.session_state.analysis_df, pd.DataFrame) and not st.session_state.analysis_df.empty:
    ...
    if selected_section == "Overview":
        ...
        with tab_time3:
            ...
            st.markdown("#### Data by Time Control Category")
        ...
        tab_eco1, tab_eco2 = st.tabs(["Frequency", "Performance"])
        with tab_eco1:
            n_openings = st.slider("Num openings:", 5, 50, 20, key="n_openings_freq")
        ...
        with tab_eco2:
            min_games_opening = st.slider("Min games:", 1, 25, 5, key="min_games_perf")
            n_openings_perf = st.slider("Num openings by win rate:", 5, 50, 20, key="n_openings_perf")
        ...
        tab_opp1, tab_opp2 = st.tabs(["Frequency", "Elo Diff vs Result"])
        with tab_opp1:
            n_opponents = st.slider("Num opponents:", 5, 50, 20, key="n_opponents_freq")
        ...
        if not gm_games.empty:
            ...
            tab_gm1, tab_gm2, tab_gm3, tab_gm4 = st.tabs(["🏆 Summary", "📈 Rating", "📖 Openings", "👥 Opponents"])
            with tab_gm1:
